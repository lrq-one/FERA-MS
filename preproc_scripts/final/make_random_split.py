...
def main():
    ...
    if len(candidates) >= target_keep:
        keep_df = candidates.sort_values("qcbase_model_bad_score", ascending=True).head(target_keep).copy()
    else:
    ...
    keep_df.to_csv(out_split_dp / "qcbase_model_train_keep_details.csv", index=False)
    drop_df.to_csv(out_split_dp / "qcbase_model_train_drop_details.csv", index=False)
