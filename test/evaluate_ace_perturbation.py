...
def evaluate_split(
    ...
    for batch in tqdm(
        ...
        if ace_plan is not None:
            ace_batch_size = _apply_ace_plan(
                batch,
                ace_plan,
                ace_plan_offset,
            )
            ace_plan_offset += ace_batch_size
...
def _normalise_ace_key(value: object) -> str:
    return "".join(
        character
...
def _collect_ace_slots(
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            key_name = _normalise_ace_key(key)
            child_path = f"{path}.{key}"

            if (
                key_name in _ACE_KEYS
                and torch.is_tensor(child)
                and child.numel() > 0
            ):
                slots.append(
                    (
    ...
    for slot in slots:
        if slot[3] in seen_paths:
            continue

        seen_paths.add(slot[3])
        unique.append(slot)
...
def _extract_primary_ace(
    batch: object,
) -> torch.Tensor:
    slots = _collect_ace_slots(batch)
...
def _prepare_ace_plan(
    ...
    for batch in loader:
        ...
        if first_path is None:
            first_path = slots[0][3]
            print(
                "[ACE perturbation] "
                f"primary field: {first_path}"
            )

        chunks.append(
            _extract_primary_ace(batch)
        )
    ...
    if ACE_MODE == "median":
        ...
        print(
            "[ACE perturbation] "
            f"median ACE used: {median_value}"
        )
    ...
    if ACE_MODE == "shuffled":
        generator = torch.Generator()
        generator.manual_seed(
            int(ACE_SHUFFLE_SEED)
        )
        ...
        print(
            "[ACE perturbation] "
            f"shuffle seed: {ACE_SHUFFLE_SEED}"
        )
...
def main() -> None:
    ...
    global ACE_MODE
    global ACE_MEDIAN
    global ACE_SHUFFLE_SEED

    ACE_MODE = str(args.ace_mode)
    ACE_MEDIAN = args.ace_median
    ACE_SHUFFLE_SEED = int(
        args.ace_shuffle_seed
    )
    ...
    print(
        ...
        f"mode={ACE_MODE}, "
        f"median={ACE_MEDIAN}, "
        f"shuffle_seed={ACE_SHUFFLE_SEED}"
    )
