...
CANDIDATE_RERANKER_PATH = ROOT / "train/_impl/refinement_steps/candidate_reranker.py"
...
def load_candidate_assets() -> dict[str, Any]:
    ...
    required = [
        ...
        TEMPLATE_PATH, CANDIDATE_RERANKER_PATH, SPECTRUM_ALLOCATOR_PATH,
    ]
...
def build_expanded_candidate_input(
    ...
    if "dset" in spec_input.columns:
        spec_input["dset"] = "molecular_retrieval_pubchem"
...
def initialize_model(
    ...
    state = candidate_reranker.load_state_dict_any(backbone_path)
    ...
    allocator = spectrum_allocator.ResidualAllocator(
    ...
    hashes = {
        "final_peak_distillation": sha256(backbone_path),
        "candidate_reranker": sha256(reranker_path),
        "spectrum_allocator": sha256(allocator_path),
    }
    return (
...
def run_predictions(
    ...
    with torch.inference_mode():
        for session_batch_index, batch in enumerate(
            ...
            result = candidate_reranker.attach_raw_rich_features(
                backbone,
                batch,
                result,
                max_extra_dims=int(
            ...
            result = spectrum_allocator.alias_rich_feature_keys(
                result,
                extra_schema,
            )

            features = candidate_reranker.candidate_features(
                result,
                batch,
...
def run_combination(
    ...
    base_config = candidate_reranker.force_final_peak_distillation_arch(
        base_config
    )
...
def main() -> None:
    ...
    run_manifest = {
        ...
        "scope": "ours_FINAL_PEAK_DISTILLATION_CANDIDATE_RERANKER_SPECTRUM_ALLOCATOR_only",
    ...
    try:
        for split in args.splits:
            for seed in args.seeds:
                run_combination(
                    split, seed, args, assets, candidate_reranker, spectrum_allocator, device
                )
