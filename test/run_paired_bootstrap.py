...
BASELINE_FP = (
    ROOT
    / "runs/experiments/paired_bootstrap_bootstrap/inputs"
...
seed_averaged_fp = (
    OUT_DIR
    / "paired_bootstrap_per_spectrum_seed_averaged.csv.gz"
)

per_molecule_fp = (
    OUT_DIR
    / "paired_bootstrap_per_molecule_scores.csv.gz"
)

pairwise_molecule_fp = (
    OUT_DIR
    / "paired_bootstrap_pairwise_molecule_differences.csv.gz"
)

macro_fp = (
    OUT_DIR
    / "paired_bootstrap_bootstrap_macro.csv"
)

micro_fp = (
    OUT_DIR
    / "paired_bootstrap_bootstrap_clustered_micro.csv"
)

all_summary_fp = (
    OUT_DIR
    / "paired_bootstrap_bootstrap_all.csv"
)

distribution_fp = (
    OUT_DIR
    / "paired_bootstrap_bootstrap_distributions.npz"
)

audit_fp = (
    OUT_DIR
    / "paired_bootstrap_bootstrap_audit.json"
)
...
if (
    ...
    print(
        "PAIRED_BOOTSTRAP_MOLECULE_PAIRED_BOOTSTRAP_COMPLETE"
    )
else:
    raise RuntimeError(
        "实验8结果行数不闭合"
