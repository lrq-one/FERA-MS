...
DIRECT_PROPERTY_CACHE_DIR = (
    CACHE_DIR
    / "direct_property_queries_by_connectivity"
)
...
def query_mass_properties_direct(
    target_key: str,
    exact_mass: float,
) -> tuple[
    list[tuple],
    ...
    cache_path = (
        ...
        / (
            f"{safe_key}_"
            f"{exact_mass:.6f}_"
            f"{PPM:g}ppm_properties.json.gz"
        )
    )
    ...
    if cache_hit:
        with gzip.open(
            ...
            cache_payload = json.load(
                handle
            )

        properties = cache_payload.get(
            "properties",
            [],
        )
    ...
    for item in properties:
        ...
        exact_mass_value = item.get(
            "ExactMass"
        )

        try:
            exact_mass_value = float(
                exact_mass_value
            )
        except (
            TypeError,
    ...
    property_rows = [
        normalized_by_cid[cid]
        for cid in cids
    ]

    print(
        "    direct properties: "
        f"rows={len(property_rows)} "
        f"cache_hit={int(cache_hit)} "
        f"sqlite_before={len(present_before)} "
        f"sqlite_after={len(present_after)}",
        flush=True,
    )
...
manifest = {
    ...
    "direct_property_cache": str(
        DIRECT_PROPERTY_CACHE_DIR
    ),
    "property_retrieval_mode": (
        "single direct exact-mass-range "
        "property request per target"
    ),
}
