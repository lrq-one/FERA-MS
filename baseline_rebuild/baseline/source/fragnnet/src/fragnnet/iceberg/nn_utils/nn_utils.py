...
import fragnnet.iceberg.nn_utils.dgl_modules as dgl_mods
...
class MLPBlocks(nn.Module):
	...
	def safe_apply_bn(self, x, bn):
		"""transpose and untranspose after linear for 3 dim items to use
		batchnorm"""
		temp_shape = x.shape
		if len(x.shape) == 2:
			return bn(x)
...
def _gen_mask(lengths_x, lengths_y, max_len_x, max_len_y):
	...
	x_mask = th.arange(max_len_x, device=device).unsqueeze(0) < lengths_x.unsqueeze(
		1
	)
	# y_mask: (batch_size, max_len_y)
	y_mask = th.arange(max_len_y, device=device).unsqueeze(0) < lengths_y.unsqueeze(
		1
	)
...
def pad_packed_tensor(input, lengths, value):
	...
	device = input.device
	if not isinstance(lengths, th.Tensor):
	...
	# Row shifts
	row_shifts = th.cumsum(max_len - lengths, 0)
	...
	cumsum_inds = th.cumsum(index, 0) - 1
...
def random_walk_pe(g, k, eweight_name=None):
	...
	if eweight_name:
		vals = g.edata[eweight_name].squeeze().numpy()
	else:
		vals = np.ones([num_edges],dtype=np.float32)
	A = sparse.csr_matrix((vals,indices),shape=(num_nodes,num_nodes))
	RW = A / (A.sum(axis=1).reshape(-1,1) + 1e-30)
	# Iterate for k steps
	PE = [RW.diagonal()]
	RW_power = RW.copy()
	for _ in range(k - 1):
