...
from fragnnet.iceberg.common import chem_utils as chem_utils
from fragnnet.utils.data_utils import rdkit_import
...
class FragmentEngine(object):
	...
	def __init__(
		...
		"""
		# do import here (to avoid warnings)
		rdkit_Chem = rdkit_import("rdkit.Chem")[0]
		...
		if mol_str_type == "smiles":
			...
			if self.mol is None:
				return
			self.inchi = rdkit_Chem.MolToInchi(self.mol)
			self.mol = rdkit_Chem.MolFromInchi(self.inchi)
			...
			if self.mol is None:
				return
			self.smiles = rdkit_Chem.MolToSmiles(self.mol)
		else:
		...
		rdkit_Chem.Kekulize(self.mol, clearAromaticFlags=True)
		...
		self.atom_weights = np.array(
			[chem_utils.ELEMENT_TO_MASS[i] for i in self.atom_symbols]
		)
		self.atom_weights_h = (
	...
	def wl_hash(
		...
		Return:
			new_fragment
		"""
		# Step 1: Define initial hashes by symbol
		cur_hashes = [
			f"{i}"
