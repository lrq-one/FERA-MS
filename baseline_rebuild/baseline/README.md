...
| ICEBERG | ICEBERG-ACE | `https://github.com/coleygroup/ms-pred` | Formal-run source snapshot retained | `source/fragnnet/LICENSE` (BSD-2-Clause) | `source/fragnnet/src/fragnnet/iceberg/` plus shared runtime | ACE cohort/config and restored CE-gated source; removal patch retained under `source/fragnnet/patches/` | `iceberg/configs/locked.yml` |
