...
def _spectrum_objective_ce_vector(batch, batch_size, device, dtype):
    ce = batch.get("spec_ce", None)
    if ce is None:
        raise RuntimeError("spectrum objective requires spec_ce in batch")
    ...
    if ce_batch_idxs is None:
        if int(ce.numel()) == 1:
            return ce.expand(bs)
        raise RuntimeError(f"spectrum objective cannot map spec_ce shape={tuple(ce.shape)} to batch_size={bs}")

    ce_batch_idxs = ce_batch_idxs.reshape(-1).to(device=device).long()
    if int(ce_batch_idxs.numel()) != int(ce.numel()):
        raise RuntimeError(
            f"spectrum objective spec_ce/spec_ce_batch_idxs mismatch: {tuple(ce.shape)} vs {tuple(ce_batch_idxs.shape)}"
        )
...
def _spectrum_objective_ce_weight(ce_vec, hparams):
    mid_thr = float(getattr(hparams, "spectrum_objective_mid_ce_threshold", 20.0))
    high_thr = float(getattr(hparams, "spectrum_objective_high_ce_threshold", 40.0))

    low_w = float(getattr(hparams, "spectrum_objective_low_ce_weight", 0.5))
    mid_w = float(getattr(hparams, "spectrum_objective_mid_ce_weight", 2.5))
    high_w = float(getattr(hparams, "spectrum_objective_high_ce_weight", 5.0))
...
def spectrum_objective_ce_weighted_spectrum_loss(
    batch,
    batch_size,
    hparams,
    true_mzs,
    ...
    """
    spectrum objective: direct spectrum-level CE-weighted loss.
    ...
    mz_max = float(getattr(hparams, "spectrum_objective_mz_max", getattr(hparams, "mz_max", 1500.0)))
    mz_bin_res = float(getattr(hparams, "spectrum_objective_mz_bin_res", 0.01))
    log_min = float(getattr(hparams, "log_min", 1.0e-9))

    cos_w = float(getattr(hparams, "spectrum_objective_cos_weight", 1.0))
    jss_w = float(getattr(hparams, "spectrum_objective_jss_weight", 0.35))
    ...
    ce_vec = _spectrum_objective_ce_vector(batch, batch_size, device=device, dtype=dtype)
    ce_weight = _spectrum_objective_ce_weight(ce_vec, hparams)
