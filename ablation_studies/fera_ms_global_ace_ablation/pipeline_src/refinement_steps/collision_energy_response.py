...
def override_cfg_collision_energy_response(cfg, args):
    ...
    # ===== collision_energy_response_refinement: CE-response joint residual =====
    ...
    # ===== bounded_residual_flow_refinement: CEFlowFrag refined_variant =====
...
def freeze_for_collision_energy_response(model, train_formula_composition=False, train_refiner=False, train_formula_module=False, train_render_gate=False, train_frag_rep=False, train_ce_flowfrag=False):
    ...
    print("[collision_energy_response_refinement] trainable params:", n_train, "/", n_total)
    print("[collision_energy_response_refinement] trainable prefixes:", allow)
    print("[collision_energy_response_refinement] first trainable names:")
...
def main():
    ...
    # Slightly stronger mid/high emphasis than formula_composition_refinement.
    ...
    cfg = override_cfg_collision_energy_response(cfg, args)
    ...
    print("[collision_energy_response_refinement] missing keys:", len(missing))
    for k in missing[:40]:
        print("  missing:", k)
    if len(missing) > 40:
        print("  ...")
    print("[collision_energy_response_refinement] unexpected keys:", len(unexpected))
    ...
    print("\n===== collision_energy_response_refinement BEFORE VAL =====")
    print("[collision_energy_response_refinement] global cos:", formula_composition.global_metric(baseline_val, "cos"))
    print("[collision_energy_response_refinement] global jss:", formula_composition.global_metric(baseline_val, "jss"))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        losses = []
        pbar = tqdm(train_dl, desc=f"collision_energy_response_refinement train epoch={epoch}")
        ...
        logs.append(row)
        print("[collision_energy_response_refinement train]", row)
        ...
        print(f"[collision_energy_response_refinement] epoch={epoch} score={score:.6f}")
        print("[collision_energy_response_refinement] global cos:", formula_composition.global_metric(val_sum, "cos"))
        print("[collision_energy_response_refinement] global jss:", formula_composition.global_metric(val_sum, "jss"))
        ...
        if score > best_score:
            ...
            print("[collision_energy_response_refinement] saved best:", best_path)
    ...
    if best_path.exists():
        ...
        print("[collision_energy_response_refinement] loaded best epoch:", ckpt["epoch"], "score:", ckpt["best_score"])
    ...
    print("\n===== collision_energy_response_refinement BEST VAL =====")
    print("[collision_energy_response_refinement] best global cos:", formula_composition.global_metric(best_val, "cos"))
    print("[collision_energy_response_refinement] best global jss:", formula_composition.global_metric(best_val, "jss"))
    ...
    if args.eval_test:
        ...
        print("\n===== collision_energy_response_refinement BEST TEST =====")
        print("[collision_energy_response_refinement] best global cos:", formula_composition.global_metric(best_test, "cos"))
        print("[collision_energy_response_refinement] best global jss:", formula_composition.global_metric(best_test, "jss"))
