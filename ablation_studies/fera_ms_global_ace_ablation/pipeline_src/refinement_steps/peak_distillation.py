...
def override_cfg(cfg, args):
    ...
    # Keep old false-mass aux off in final_peak_distillation. We do our own candidate-level false loss.
...
def freeze_model(model, args):
    ...
    print(f"[final_peak_distillation] trainable params: {trainable} / {total}")
    print(f"[final_peak_distillation] allowed prefixes: {allow}")
...
def build_spectrum_objective_spectrum_loss(results, batch, model, args):
    ...
    This is intentionally inside the final_peak_distillation training script so the model architecture
    stays exactly aligned with final_peak_distillationA/candidate reranker.
    """
    pred_mzs = results["pred_mzs"]
    pred_logp = results["pred_logprobs"]
    pred_prob = pred_logp.exp()
...
def main():
    ...
    # final_peak_distillation oracle losses.
    ...
    print("[final_peak_distillation] missing keys:", len(missing))
    for x in missing[:30]:
        print("  missing:", x)
    print("[final_peak_distillation] unexpected keys:", len(unexpected))
    ...
    print_eval("final_peak_distillation BEFORE VAL", before_table)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        pbar = tqdm(train_dl, desc=f"final_peak_distillation train epoch={epoch}")
        ...
        print("[final_peak_distillation train]", row)
        ...
        print_eval(f"final_peak_distillation VAL epoch={epoch} score={score:.6f}", val_table)
        ...
        if score > best_score:
            ...
            print("[final_peak_distillation] saved best:", out_dir / "final_peak_distillation_best_state.pt")

    print("[final_peak_distillation] loaded best epoch:", best_epoch, "score:", best_score)
    ...
    print_eval("final_peak_distillation BEST VAL", best_val)
    ...
    if args.eval_test:
        ...
        print_eval("final_peak_distillation BEST TEST", best_test)
