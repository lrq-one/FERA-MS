...
def freeze_for_k3b(model, train_formula_module=False, train_refiner=False):
    ...
    print("[formula_composition_refinement] trainable params:", n_train, "/", n_total)
    print("[formula_composition_refinement] trainable prefixes:", allow)
    print("[formula_composition_refinement] first trainable names:")
    for n in names[:30]:
        print("  ", n)
    if len(names) > 30:
        print("  ...")
...
def main():
    ...
    print("[formula_composition_refinement] missing keys:", len(missing))
    for k in missing[:20]:
        print("  missing:", k)
    print("[formula_composition_refinement] unexpected keys:", len(unexpected))
    ...
    print("\n===== formula_composition_refinement BEFORE VAL =====")
    print("[formula_composition_refinement] global cos:", global_metric(baseline_val, "cos"))
    print("[formula_composition_refinement] global jss:", global_metric(baseline_val, "jss"))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        losses = []
        pbar = tqdm(train_dl, desc=f"formula_composition_refinement train epoch={epoch}")
        ...
        print("[formula_composition_refinement train]", row)
        ...
        print(f"[formula_composition_refinement] epoch={epoch} score={score:.6f}")
        print("[formula_composition_refinement] global cos:", global_metric(val_sum, "cos"))
        print("[formula_composition_refinement] global jss:", global_metric(val_sum, "jss"))
        ...
        if score > best_score:
            ...
            print("[formula_composition_refinement] saved best:", best_path)
    ...
    if best_path.exists():
        ...
        print("[formula_composition_refinement] loaded best epoch:", ckpt["epoch"], "score:", ckpt["best_score"])

    best_val = eval_buckets(model, val_dl, device, "val")
    best_val.to_csv(out_dir / "formula_composition_best_val.csv", index=False)

    print("\n===== formula_composition_refinement BEST VAL =====")
    print("[formula_composition_refinement] best global cos:", global_metric(best_val, "cos"))
    print("[formula_composition_refinement] best global jss:", global_metric(best_val, "jss"))
    ...
    if args.eval_test:
        ...
        print("\n===== formula_composition_refinement BEST TEST =====")
        print("[formula_composition_refinement] best global cos:", global_metric(best_test, "cos"))
        print("[formula_composition_refinement] best global jss:", global_metric(best_test, "jss"))
