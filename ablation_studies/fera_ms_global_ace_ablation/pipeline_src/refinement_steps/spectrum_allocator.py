...
def alias_rich_feature_keys(res, extra_schema):
    ...
    老的 candidate_reranking regressor pack 可能记录的是 candidate_reranker_frag_rich_feats。
...
def eval_split(base, allocator, regressor, extra_schema, dl, device, candidate_reranker, args, split):
    ...
    for batch_i, batch in enumerate(tqdm(dl, desc=f"eval {split}")):
...
def main():
    ...
    for epoch in range(
        ...
        if val_cos > best_val_cos:
            ...
            print("[spectrum allocation] saved best:", out_dir / "spectrum_allocator.pt")

    pack = th.load(out_dir / "spectrum_allocator.pt", map_location=device, weights_only=False)
