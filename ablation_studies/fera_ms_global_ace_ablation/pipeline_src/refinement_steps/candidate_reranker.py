...
def force_final_peak_distillation_arch(cfg):
    ...
    # Global-only neural control: preserve final_peak_distillation non-CE features, never revive
    # local CE response/flow consumers in downstream feature extraction.
    cfg["use_ce_response_scorer"] = False
...
def attach_raw_rich_features(base, batch, results, max_extra_dims=96, bin_res=0.01):
    ...
    Feature sources:
      A) old/raw peak-entry features from candidate_reranking
    ...
    try:
        ...
        # -------- A) candidate_reranking stable peak-entry features --------
        stable_peak_keys = [
            "candidate_reranker_peak_rich_feats",
...
@th.no_grad()
def eval_split(base, regressor, extra_schema, dl, device, args, split, alpha, return_detail=False):
    base.eval()
    rows = []

    for batch_i, batch in enumerate(tqdm(dl, desc=f"eval {split} alpha={alpha}")):
        ...
        if int(getattr(args, "max_eval_batches", 0)) > 0 and batch_i + 1 >= int(args.max_eval_batches):
            break
    ...
    return (table, df) if return_detail else table
...
def main():
    ...
    # Isolated smoke never initializes or reads the locked test split.
