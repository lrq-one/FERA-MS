...
import pandas as pd
ROOT=Path(__file__).resolve().parents[3]; ABL=ROOT/'ablation_studies/fera_ms_global_ace_ablation'
rows=[]
for seed in (42,43,44):
 for split in ('val','test'):
  p=ABL/'evaluation'/f'seed_{seed}'/f'spectrum_allocation_{split}_metrics.csv'
  if not p.is_file(): raise FileNotFoundError(p)
  df=pd.read_csv(p); g=df.loc[df.ce_bucket.eq('global')].iloc[0]
  rows.append({'seed':seed,'stage':'spectrum_allocation','split':split,'spec_count':int(g.spec_count),'spectrum_micro_cbin':float(g.cos),'spectrum_micro_jss':float(g.jss)})
