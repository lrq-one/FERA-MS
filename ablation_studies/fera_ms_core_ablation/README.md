...
This directory retains the valid **without chemical candidate reranking** control. It reuses each completed mainline R160 checkpoint, removes the R172D contribution by using the locked zero-weight route, retrains the spectrum allocator with the mainline hyperparameters, selects on validation, and evaluates test once.

```bash
bash ablation_studies/fera_ms_core_ablation/scripts/00_preflight.sh
