...
do
    ...
    do
        ...
        if [ -s "$OUT/spectrum_allocator_best_test.csv" ] \
           && [ -s "$OUT/spectrum_allocator_allocator_best.pt" ]; then
        ...
        python - \
          "$OUT/spectrum_allocator_best_test.csv" \
          "$EXPECTED_TEST_COUNT" \
