...
| Paper cohort restoration | `preproc_scripts/final/make_random_split.py` | `config/paper_experiment_identity.json` | archived licensed `safe19707` tables, random assignment, and required DAG cache | byte-verified 19,659-spectrum/2,274-molecule cohort | excludes only three training molecules with unavailable required depth-3 DAGs |
| Formal DAG materialization | `preproc_scripts/final/materialize_formal_dag_cache.py` | locked DAG manifest | rebuilt or archived formal `safe19659` `nl_v1` DAGs plus cohort IDs | formal training DAG directory | selects/links only; never relabels the different `safe19707...base` cache |
| Random split | `preproc_scripts/final/make_random_split.py` | archived assignment plus SHA-256 contract | restored paper cohort | exact molecule-disjoint random IDs | preserves the formal assignment; no new random draw |
...
| Backbone | `python train/train.py base` | `config/train.yml` → `runs/_config/` | processed data, split, DAGs | GINE/cut-chemistry checkpoint | 40-epoch initialization, then 10-epoch continuation; validation selects between their best checkpoints |
| Control fitting | `python train/train.py control` | generated control config | validation-selected backbone | control checkpoint | 5 epochs initialized from the selected backbone; validation only for selection |
| CE/formula/H refinement | `python train/train.py refinement` | locked refinement settings in `config/train.yml` | control checkpoint | formula-to-peak refinement checkpoints | sequential validation selection; CE-flow is retained only when it improves the prespecified validation metric, otherwise the preceding checkpoint is retained |
